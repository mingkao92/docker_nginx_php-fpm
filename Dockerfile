...
RUN set -eux; \
	...
	{ \
		echo 'https://mirrors.aliyun.com/alpine/v3.10/main'; \
		echo 'https://mirrors.aliyun.com/alpine/v3.10/community'; \
...
# start install php-fpm
# ensure www-data user exists
# 82 is the standard uid/gid for "www-data" in Alpine
RUN set -eux; \
# group www-data maybe exists, just skip
...
COPY php-fpm/base/docker-php-* /usr/local/bin/
...
# enable php
COPY nginx/enable-php.conf /etc/nginx/

# add vhosts
COPY nginx/vhosts.conf /etc/nginx/conf.d/
...
# modify php ini
	sed -i \
	    ...
	    -e 's/upload_max_filesize = 2M/upload_max_filesize = 10M/g' \
	    -e 's/post_max_size = 8M/post_max_size = 16M/g' \
	    -e 's/max_execution_time = 30/max_execution_time = 300/g' \
	    -e 's/;cgi.fix_pathinfo=1/cgi.fix_pathinfo=0/g' \
	    -e 's/disable_functions =/disable_functions = passthru,exec,system,chroot,chgrp,chown,shell_exec,popen,ini_alter,ini_restore,dl,openlog,syslog,readlink,symlink,popepassthru,stream_socket_server/g' \
    	"$PHP_INI_DIR/php.ini"; \
...
# add monitor s6-overlay
ADD s6-overlay/s6-overlay-amd64.tar.gz /
